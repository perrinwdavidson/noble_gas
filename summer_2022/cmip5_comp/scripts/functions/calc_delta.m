...
%%	load data
%	u10 ::
data{1} = load(fullfile(output_path, 'cmip5', 'cmip5_u10_lgm_uvic_monthly'));
data{2} = load(fullfile(output_path, 'cmip5', 'cmip5_u10_pic_uvic_monthly')); 

%	v10 ::
data{3} = load(fullfile(output_path, 'cmip5', 'cmip5_v10_lgm_uvic_monthly')); 
data{4} = load(fullfile(output_path, 'cmip5', 'cmip5_v10_pic_uvic_monthly')); 

%%	calculate multiplicative factor
%	get number of models and months ::
NUMMOD = size(cmip5_u10_lgm_uvic_monthly{1, 1}, 3); 
NUMMON = size(cmip5_u10_lgm_uvic_monthly, 2); 
NUMDATA = size(data, 2);

%	preallocate space ::
factor_lgm_pic = NaN(size(cmip5_u10_lgm_uvic_monthly)); 
...
%%	save data
save(fullfile(output_path, 'wind_factor', 'factor_lgm_pic.mat'), 'factor_lgm_pic.mat');
